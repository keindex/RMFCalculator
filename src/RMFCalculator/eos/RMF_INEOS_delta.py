r"""
RMF_INEOS_delta.py - 含同位旋矢量-标量介子 $\delta$ 的无限核物质状态方程计算
...
约定:
- 本模块采用与 ``RMF_INEOS`` / ``RMF_betaEOS`` 相同的 "大 $g_\rho$" 约定,
...
# 兼容 13 元 theta13 与旧 10 元 theta 的参数校验函数
def _is_theta13(theta):
    return np.asarray(theta).shape == (13,)


def _validate_theta13(theta):
    r"""
    校验并返回 13 元含 $\delta$ 介子的 RMF 参数向量。
    ...
    try:
        ...
        raise TypeError("RMF_INEOS_delta expects a length-13 numeric RMF theta13.") from exc
    if theta_array.shape != (13,):
        raise TypeError("RMF_INEOS_delta expects a length-13 numeric RMF theta13.")
    return theta_array
...
def initial_values(rho, theta13, proton_fraction=0.5):
    r"""
    ...
    # $\delta$ 场线性近似: $g_\delta(n_p^s - n_n^s)/m_\delta^2$
...
def functie(x, args):
    ...
    Returns:
        list: 6 个残差的平方 (供 scipy.optimize.root 的 LM 方法使用)
    """
    (m_sig, m_w, m_rho, m_delta, g_sigma, g_omega, g_rho,
...
def Energy_density_Pressure(x, rho, theta13, x_p=0.5):
    r"""
    计算含 $\delta$ 介子的无限核物质能量密度与压强。
    ...
    # $\sigma$ 场能量 (含 $\delta$-$\sigma$ 耦合的势能项)
    sigma_terms = (
        0.5 * (sigma * m_sig)**2
    ...
    # $\delta$ 场能量: $\frac12 m_\delta^2 \delta^2$
...
def compute_INEOS_delta(theta13, x_p=0.5, n_points=124, dt=0.05, rho_0=0.1505):
    r"""
    计算含 $\delta$ 介子的无限核物质状态方程。

    Args:
        theta13 (array): 13 元 RMF 参数向量 (含 $\delta$ 参数)
    ...
    Returns:
        ndarray: $(6, n_\text{points})$ 数组, 行:
            ...
            [4] 质子化学势 $\mu_p$ (fm$^{-1}$)
            [5] 质子分数 $x_p$
        (与 ``compute_INEOS`` 输出一致; 乘以 ``fm_MeV`` 即得 MeV/fm$^3$)
    """
...
if __name__ == "__main__":
    ...
    EoS_test = compute_INEOS_delta(theta13_test, x_p=0.5, n_points=20, dt=0.05)
